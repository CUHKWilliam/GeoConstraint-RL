...
class ReKepOGEnv:
    ...
    def segment(self, obj_part, rekep_program_dir=None):
        ...
        mask = self.segmentor.segment(image_path="rgb_obs.png", obj_description=obj_part, rekep_program_dir=rekep_program_dir)
        return mask
...
from remote.robot_controller import Robot
from remote.camera import Camera
import utils
...
class RealEnv:
    def __init__(self, config, verbose=False):
        ...
        self.video_tmp = []
        if TRACKING:
            self.cotracker =  torch.hub.load("facebookresearch/co-tracker", "cotracker3_offline").to("cuda:1")
        self.is_grasping = False
        self.moving_part_names = []
        self.part_to_pts_dict = []
        self.part_to_pts_dict_simulation = None
        self.camera = Camera(
            lock_file_path = "/data/tmp/lock2.txt",
            data_file_path = "/data/tmp/communication2.pkl"
        )
        self.robot = Robot(
            lock_file_path = "/data/tmp/lock.txt",
            data_file_path = "/data/tmp/communication.pkl"
        )
        self.part_lists = []
        self.last_cam_obs = None
        self.APPROACH0 = self.robot.APPROACH0
        self.BINORMAL0 = self.robot.BINORMAL0
        self.part_to_segm_dict = {}
        self.previous_pose = self.get_ee_pose()
        self.pose0_grasp = None
        self.current_pose = self.previous_pose.copy()
        self.UPDATE_PTS_TRACKANYTHING = False

    def get_moving_part_names(self, ):
        return self.moving_part_names

    def get_cam_obs(self, use_last=False, record=True):
        if self.last_cam_obs is None or not use_last:
            cam_obs = self.camera.get_cam_obs()
            if record:
                self.last_cam_obs = [[], cam_obs]
        return self.last_cam_obs

    def get_cam_rgb(self, ):
        return self.camera.get_cam_rgb()
    ...
    def update_part_to_pts_dict(self):
        part_to_pts_dict = self.part_to_pts_dict
        part_to_segm_dict = self.part_to_segm_dict
        part_to_pts_dict_latest = {}
        for i, part in enumerate(self.part_to_pts_dict[0].keys()):
            if "gripper" in part:
                if "approach" in part:
                    start = self.robot.get_ee_pos()
                    approach = self.robot.get_approach()
                    approach = approach / np.linalg.norm(approach)
                    end = start + approach * 0.3
                    part_to_pts_dict_latest[part] = np.linspace(start, end, 5)
                elif "binormal" in part:
                    start = self.robot.get_ee_pos()
                    binormal = self.robot.get_binormal()
                    binormal = binormal / np.linalg.norm(binormal)
                    end = start + binormal * 0.05
                    part_to_pts_dict_latest[part] = np.linspace(start, end, 5)
                else:
                    part_to_pts_dict_latest[part] = self.robot.get_ee_pos()[None, :]
            else:
                image_debug0 = self.video_tmp[0]['rgb'].copy()
                image_debug1 = self.video_tmp[-1]['rgb'].copy()
                part_pts = part_to_pts_dict[-1][part]
                part_segm = part_to_segm_dict[part]
                part_pts_2d = np.stack(np.where(part_segm>0), axis=-1)[:, ::-1]
                video = self.video_tmp
                video = [a_video['rgb'] for a_video in video]
                MAX_LEN_VIDEO = 30
                video = np.stack(video, axis=0)
                if len(video) > MAX_LEN_VIDEO:
                    indices = np.linspace(0, len(video) - 2, MAX_LEN_VIDEO - 1)
                ...
                video = torch.from_numpy(video).float().to("cuda:1")
                video = video.unsqueeze(0).permute(0, 1, 4, 2, 3).contiguous()
                queries = torch.cat([torch.tensor([0] * part_pts_2d.shape[0]).unsqueeze(-1), torch.from_numpy(part_pts_2d.copy())], dim=-1).to("cuda:1").float().unsqueeze(0)
                pred_tracks, pred_visibility = self.cotracker(video, queries=queries)
                # for ind in range(0, video.shape[1] - self.cotracker.step, self.cotracker.step):
                #     pred_tracks, pred_visibility = self.cotracker(video_chunk=video[:, ind : ind + self.cotracker.step * 2], queries=queries, is_first_step=False)
                part_pts_2d2 = pred_tracks[0, -1, :, :].detach().cpu().numpy().astype(np.int64)
                part_segm2 = np.zeros_like(part_segm)
                part_segm2[part_pts_2d2[:, 1], part_pts_2d2[:, 0]] = 1
                part_to_segm_dict[part] = part_segm2
                part_pts2 = self.mask_to_pc(part_segm2)
                part_to_pts_dict_latest[part] = part_pts2
                image_debug0[part_pts_2d[:, 1], part_pts_2d[:, 0], :] = np.array([255, 0, 0])
                image_debug1[part_pts_2d2[:, 1], part_pts_2d2[:, 0], :] = np.array([255, 0, 0])
                ## for debug
                cv2.imwrite("tracking_debug0.png", image_debug0[:, :, ::-1])
                cv2.imwrite("tracking_debug1.png", image_debug1[:, :, ::-1])
                # import ipdb;ipdb.set_trace()
                ## end for debug
        self.video_tmp = []
        torch.cuda.empty_cache()
        ...
        part_to_pts_dict.append(part_to_pts_dict_latest)
        self.part_to_pts_dict = part_to_pts_dict
        self.part_to_segm_dict = part_to_segm_dict
        return part_to_pts_dict

    def update_part_to_pts_dict_no_track(self, ):
        part_to_pts_dict = self.part_to_pts_dict
        curr_ee_pose = self.current_pose
        if np.all(self.previous_pose == curr_ee_pose):
            return part_to_pts_dict
        part_to_pts_dict_latest = {}
        centered_part_to_pts_dict = utils._center_geometry(self.previous_pose, part_to_pts_dict, self.get_moving_part_names())
        transform = np.concatenate([T.quat2mat(curr_ee_pose[3:]), curr_ee_pose[:3][:, None]], axis=-1)
        part_to_pts_dict_latest = utils.transform_geometry(transform, centered_part_to_pts_dict, self.get_moving_part_names())
        self.part_to_pts_dict = part_to_pts_dict_latest
        self.previous_pose = curr_ee_pose
        print("update_part_to_pts_dict_no_track!")
        return part_to_pts_dict
    
    def segment(self, obj_part, rekep_program_dir=None, mask=None, use_tracking=False):
        if use_tracking:
            keypoints_dict = self.camera.get_cached_keypoints_dict(obj_part)
        cam_obs = self.last_cam_obs[1]
        rgb = cam_obs['rgb']
        cv2.imwrite("rgb_obs.png", rgb[:, :, ::-1])
        if not use_tracking or obj_part not in keypoints_dict:
            if len(self.part_to_pts_dict) > 0 and obj_part in self.part_to_pts_dict[-1].keys():
                pc = self.part_to_pts_dict[-1][obj_part]
                mask = self.part_to_segm_dict[obj_part]
            else:
                mask = self.segmentor.segment(image_path="rgb_obs.png", obj_description=obj_part, rekep_program_dir=rekep_program_dir)
                MAX_POINTS_NUM = 200
                pts_2d = np.stack(np.where(mask > 0), axis=-1)[:, ::-1]
                if len(pts_2d) > MAX_POINTS_NUM:
                    pts_2d = pts_2d[np.random.choice(np.arange(len(pts_2d)), MAX_POINTS_NUM)]
                mask2 = np.zeros_like(mask)
                mask2[pts_2d[:, 1], pts_2d[:, 0]] = 1
                mask = mask2
                if use_tracking:
                    self.camera.register_keypoints(obj_part, mask)
                pc = self.mask_to_pc(mask)
        else:
            pts_2d, pc = keypoints_dict[obj_part]
            mask2 = np.zeros_like(rgb[:, :, 0])
            pts_2d = np.clip(pts_2d, a_min=[0, 0], a_max=[rgb.shape[1] - 1, rgb.shape[0] - 1])
            mask2[pts_2d[:, 1].astype(np.int64), pts_2d[:, 0].astype(np.int64)] = 1
            mask = mask2
        return pc, mask

    def register_keypoints(self, part_lists, rekep_program_dir=None, is_grasp=None, use_tracking=False):
        part_to_pts_dict_latest = {}
        if len(self.part_to_pts_dict) > 0:
            part_lists = list(set(part_lists + list(self.part_to_pts_dict[-1].keys())))
        for obj_part in part_lists:
            ## handle the gripper in seperate codes
            if "gripper" in obj_part:
                if len(self.part_to_pts_dict) > 0 and obj_part in self.part_to_pts_dict[-1].keys():
                    pts = self.part_to_pts_dict[-1][obj_part]
                else:
                    if "approach" in obj_part:
                        start = self.robot.get_ee_pos()
                        approach = self.robot.get_approach()
                        ...
                        pts = self.robot.get_ee_pos()[None, :]
            else:
                # 1. If obj_part is moving:
                # - get point cloud pc0 from the first frame
                # - if obj_part already registered: use the registered points
                # - if obj_part is not registered and is_grasp, pc = pc0 @ np.inv(grasp_pose0).T @ grasp_pose.T
                # 2. If obj_part is not moving:
                # - If it's not tracked, segment it from the first frame
                # - If it's tracked, track it from the previous frame
                if obj_part in self.moving_part_names:
                    _use_tracking = False
                else:
                    _use_tracking = use_tracking
                pts, mask = self.segment(obj_part, rekep_program_dir=rekep_program_dir, use_tracking=_use_tracking)
                if obj_part in self.moving_part_names:
                    if obj_part not in self.part_to_pts_dict[-1].keys():
                        if self.pose0_grasp is not None and is_grasp is not None and is_grasp:
                            pts_centered = (pts - self.pose0_grasp[:3]) @ np.linalg.inv(T.quat2mat(self.pose0_grasp[3:])).T
                            pts_transformed = pts_centered @ T.quat2mat(self.current_pose[3:]).T + self.current_pose[:3]
                            pts = pts_transformed
                    else:
                        pts = self.part_to_pts_dict[-1][obj_part]
                self.part_to_segm_dict[obj_part] = mask
            part_to_pts_dict_latest[obj_part] = pts
            if len(self.part_to_pts_dict) > 0 and obj_part not in self.part_to_pts_dict[-1].keys():
                for i in range(len(self.part_to_pts_dict)):
                    self.part_to_pts_dict[i][obj_part] = pts
        self.part_to_pts_dict.append(part_to_pts_dict_latest)

    def mask_to_pc(self, mask):
        obs = self.last_cam_obs[1]
        pcs = obs['points'][mask > 0]
        pcs = process_point_cloud(pcs)
        return pcs
    ...
    def get_ee_pose_quat(self):
        ee_pose = self.robot.get_ee_pose()
        ee_euler = ee_pose[3:]
        ee_quat = R.from_euler("ZYX", ee_euler).as_quat()
        ee_pose_euler = np.concatenate([ee_pose[:3], ee_quat])
        return ee_pose_euler
    ...
    def execute_action(
        ...
        self.robot.execute_action(pose)