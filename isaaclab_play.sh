...
install_isaaclab_extension() {
    ...
    if [ -f "$1/setup.py" ]; then
        echo -e "\t module: $1"
        ${python_exe} -m pip install --editable $1 develop
    fi
}
...
${python_exe} "IsaacLab/source/standalone/workflows/rl_games/play.py" --task=Isaac-Lift-Cube-Franka-v0 --num_envs 32 --use_last_checkpoint
