...
def select_grasp_with_constraints(subgoal_poses, constraints, occlusion_func=None):
    ...
    for subgoal_pose in subgoal_poses:
        ...
        approaches.append(approach_vec)
        binormal0 = env.robot.BINORMAL0
        binormal_vec = binormal0 @ subgoal_pose_homo[:3, :3].T
        binormals.append(binormal_vec)
    ...
    approach = np.stack(approaches, axis=0)[np.argmin(costs)]
    binormal = np.stack(binormals, axis=0)[np.argmin(costs)]
    return {
